<x-app-layout :routePrefix="auth()->user()->user_type=='admin' ? 'admin' : 'company'" navBarAddBtn="Add Client" navBarAddBtnUrl="{{ route('company.clients.create') }}">
                                    ...
                                    <tbody>
                                        @forelse ($clients as $client)
                                            <tr>
                                                ...
                                                <td><p class="text-sm mb-0">{{ $client->name }}</p></td>
                                                <td><p class="text-sm mb-0">{{ $client->email ?? '—' }}</p></td>
                                                <td><p class="text-sm mb-0">{{ $client->phone_number ?? '—' }}</p></td>
                                                <td><p class="text-sm mb-0">{{ $client->age ?? '—' }}</p></td>
                                                <td><p class="text-sm mb-0">{{ $client->address ?? '—' }}</p></td>
                                                <td><p class="text-sm mb-0">{{ $client->created_at->format('d M Y') }}</p></td>
                                                    ...
                                                    <div class="d-flex justify-content-center align-items-center gap-2">
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <ul class="dropdown-menu dropdown-menu-end">
                                                                <li>
                                                                    <a href="{{ route('company.clients.show', $client->id) }}" class="dropdown-item">
                                                                        <i class="bi bi-eye me-1"></i> Show Detail
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <form action="{{ route('company.clients.destroy', $client->id) }}" method="POST"
